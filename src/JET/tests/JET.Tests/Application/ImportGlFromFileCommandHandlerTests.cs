...
/// <summary>
/// End-to-end integration test for the §3.1.b ingest pipeline:
/// xlsx file → <see cref="SylvanGlFileReader"/> → <see cref="SqliteGlRepository"/>
/// without ever loading the full row set into memory.
/// </summary>
public sealed class ImportGlFromFileCommandHandlerTests : IDisposable
{
    ...
    public ImportGlFromFileCommandHandlerTests()
    {
        _tempPath = Path.Combine(Path.GetTempPath(), $"jet-import-{Guid.NewGuid():N}.xlsx");
    ...
    {
        var (connectionString, keepAlive) = await OpenSharedMemoryAsync("gl-handler");
        await using var _ = keepAlive;

        var session = new StubSessionStore { CurrentProjectId = ProjectId };
        var handler = new ImportGlFromFileCommandHandler(
            session,
            new SylvanGlFileReader(),
            new SqliteGlRepository(connectionString, Names));
        ...
        Assert.Equal(new[] { "DocNum", "Amount" }, columns);

        await using var probe = new SqliteConnection(connectionString);
        await probe.OpenAsync();
        var rawRowCount = await probe.ExecuteScalarAsync<long>(
            $"SELECT COUNT(1) FROM {Names.Resolve(JetTable.StagingGlRawRow)} WHERE batch_id = @id;",
            new { id = batchId });
    ...
    {
        var (connectionString, keepAlive) = await OpenSharedMemoryAsync("gl-handler-no-project");
        await using var _ = keepAlive;
    ...
    private static void WriteFixture(string path)
    {
        ...
        table.Rows.Add("V001", "100");
        table.Rows.Add("V002", "200");
        table.Rows.Add("V003", "300");
