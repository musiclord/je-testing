...
/// <summary>
/// End-to-end integration test for the §3.1.d AccountMapping ingest pipeline:
/// xlsx file → <see cref="SylvanGlFileReader"/> → <see cref="SqliteAccountMappingRepository"/>
/// without ever loading the full row set into memory.
/// </summary>
public sealed class ImportAccountMappingFromFileCommandHandlerTests : IDisposable
{
    private const string ProjectId = "00000000-0000-0000-0000-0000000000ff";
    private static readonly ISchemaNames Names = new SqliteSchemaNames();
    private readonly string _tempPath;

    public ImportAccountMappingFromFileCommandHandlerTests()
    {
        _tempPath = Path.Combine(Path.GetTempPath(), $"jet-acctmap-import-{Guid.NewGuid():N}.xlsx");
    ...
    {
        var (connectionString, keepAlive) = await OpenSharedMemoryAsync("acctmap-handler");
        await using var _ = keepAlive;

        var session = new StubSessionStore { CurrentProjectId = ProjectId };
        var handler = new ImportAccountMappingFromFileCommandHandler(
            session,
            new SylvanGlFileReader(),
            new SqliteAccountMappingRepository(connectionString, Names));
        ...
        Assert.Equal(new[] { "AccountNo", "Category" }, columns);

        await using var probe = new SqliteConnection(connectionString);
        await probe.OpenAsync();
        var rawRowCount = await probe.ExecuteScalarAsync<long>(
            $"SELECT COUNT(1) FROM {Names.Resolve(JetTable.StagingAccountMappingRawRow)} WHERE batch_id = @id;",
            new { id = batchId });
    ...
    {
        var (connectionString, keepAlive) = await OpenSharedMemoryAsync("acctmap-handler-no-project");
        await using var _ = keepAlive;
    ...
    {
        using var table = new DataTable("AccountMapping");
        table.Columns.Add("AccountNo", typeof(string));
        table.Columns.Add("Category", typeof(string));
        table.Rows.Add("1101", "Cash");
        table.Rows.Add("1102", "Receivable");
        table.Rows.Add("4001", "Revenue");
