...
- `ExcelGlFileReader` / `CsvGlFileReader` (用 ClosedXML 或 Sylvan.Data.Excel)
- `ClosedXmlWorkPaperExporter`
...
**單次執行只會用一個 Provider**。Provider 可由使用者在 UI 切換（重啟後生效），或由「資料量門檻」觸發 (例如 GL 超過 500 萬筆自動建議切 SQL Server)。
...
└── JET/
    ...
    └── Infrastructure/
        ...
        └── Exporting/              # ClosedXmlWorkPaperExporter

tests/JET.Tests/                    # xUnit
    ├── Domain/                     # Rule spec 單元測試 (純邏輯)
    ├── Application/                # Handler 測試 (mock repository)
    └── Infrastructure/             # 每 Provider 的 integration test
```

> **目前實際狀態**：`src/JET/JET/` 只有 `Program.cs` / `Form1.cs` / `Form1.Designer.cs` / `JET.csproj`。上表為接下來要逐步落地的目標結構。
